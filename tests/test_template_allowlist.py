...
from custom_components.jinjaboard.template_allowlist import is_path_authorized
...
async def test_empty_allowlist_denies_everything(hass: HomeAssistant) -> None:
    ...
    assert is_path_authorized(hass, path) is False
...
async def test_exact_file_entry_matches_only_that_file(hass: HomeAssistant) -> None:
    ...
    assert is_path_authorized(hass, resolve_config_path(hass, "home.yaml.j2")) is True
    assert is_path_authorized(hass, resolve_config_path(hass, "other.yaml.j2")) is False
...
async def test_directory_entry_matches_nested_files(hass: HomeAssistant) -> None:
    ...
    assert (
        is_path_authorized(hass, resolve_config_path(hass, "dashboards/home.yaml.j2"))
        is True
    )
    assert (
        is_path_authorized(
    ...
    assert is_path_authorized(hass, resolve_config_path(hass, "other/home.yaml.j2")) is False
...
async def test_directory_entry_does_not_match_sibling_with_shared_prefix(
    ...
    assert (
        is_path_authorized(hass, resolve_config_path(hass, "dashboards-backup/home.yaml.j2"))
        is False
    )
...
async def test_stale_entry_outside_config_dir_is_ignored(hass: HomeAssistant) -> None:
    ...
    assert is_path_authorized(hass, resolve_config_path(hass, "home.yaml.j2")) is False
